...
namespace Engine
{
	Depth::Depth(RenderInterface* renderer,VkExtent2D extent)
	{
		...
		CreateImage(extent, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
	...
	void Depth::endSingleTimeCommands(VkCommandBuffer commandBuffer)
	{
	...
	void Depth::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
	...
	void Depth::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
	{
		...
		if (vkCreateImageView(renderer->logicalDevice, &viewInfo, nullptr, &depthImageView) != VK_SUCCESS) {
	...
	VkFormat Depth::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		...
		throw std::runtime_error("Failed to find format for depth");
	}

	void Depth::CreateImage(VkExtent2D extent, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
	{
